...
namespace Salman_ProjectApi.Controllers
{
    ...
    [ApiController]
    public class SeedController(GoldenWorldCityContext context, IHostEnvironment environment,
        UserManager<AppUser> userManager) : ControllerBase
    {
...

        {
            ...
            List<worldcitycsv> records = csv.GetRecords<worldcitycsv>().ToList();
            foreach (worldcitycsv record in records)
            {
        ...
        public async Task<IActionResult> ImportCitiesAsync()
        {
            ...
            using (CsvReader csv = new(reader, config))
            {
                IEnumerable<worldcitycsv>? records = csv.GetRecords<worldcitycsv>();
                foreach (worldcitycsv record in records)
                {
        ...
        {
            (string Name, string Email) = ("Salman", "Salman@gmail.com");
            ...
            IdentityResult result = await userManager.CreateAsync(user, "Csun2024$");
